...
//mutex variaveis globais compartilhadas
pthread_mutex_t mutex;
int ids[MAX_CONNECTIONS] = {};
int sockets[MAX_CONNECTIONS] = {};
lista ls;

//thread para lidar com a conexão com um cliente
struct threadParam {
    pthread_mutex_t *mutex;
    int sock;
} typedef threadParam;
...
void * HandleThreadTCPClient(void * param){
    ...
    while (!isComplete){
        ...
        if(operation.operation_type == NOVA_CONEXAO) {
            int freeId = -1;
            ...
            // envia o post para todos que estão inscritos
            operation.server_response = TRUE;
            for (int i = 0; i < MAX_CONNECTIONS; i++){
                if (tpc->inscritos[i]){
            ...
            // já esta inscrito
            if (tpc->inscritos[operation.client_id - 1]){
                operation.server_response = TRUE;
                ...
                tpc->inscritos[operation.client_id - 1] = TRUE;
        ...
        else if (operation.operation_type == DESCONECTAR_SERVIDOR){
            isComplete = TRUE;
            ...
            int i = 0;
            while (iterator != NULL){
                iterator->inscritos[operation.client_id - 1] = FALSE;
                iterator = iterator->next;
            };
            ids[operation.client_id - 1] = FALSE;
            ...
            topico *tpc = find_topico(&ls, operation.topic);
            if (tpc != NULL)
                tpc->inscritos[operation.client_id - 1] = FALSE;
            pthread_mutex_unlock(t_param.mutex);
